...
const CollapsibleContainer = ({ heading, workLocation, city, employmentType, childItems, borderColor }) => {
    ...
    return (
        <div className={``}>
            ...
            <div className={`w-full border-[3px] lg:border-4 ${borderColor} rounded-xl overflow-hidden ${isOpen ? 'bg-transparent' : ''}`}>
                {/* Header section */}
                <div
                    onClick={toggleOpen}
                ...
                >
                    ...
                    <div className="flex items-center">
                        <img
                            src={caretIcon}
                            alt="Caret"
                            className={`w-6 lg:w-8 svg-neon aspect-square transform transition-transform duration-500 ${isOpen ? 'rotate-180' : ''}`}
                            loading='lazy'
                        />
                    </div>
                </div>
                                    ...
                                    >
                                        <div className="flex flex-col">
                                            <div className="flex gap-x-1 lg:gap-x-4">
                                                <img src={playIcon} className="w-3 svg-neon" alt="" loading='lazy' />
                                                <Heading
                                                    text={item.jobType}
                                                    color="text-black"
                                                    fontWeight='font-medium'
