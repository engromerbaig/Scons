...
const PackageCard = ({ packageInfo }) => {
  ...
  return (
<div
  className="group relative bg-white rounded-xl shadow-xl w-full py-10 px-10 flex flex-col justify-between h-[650px] xl:h-[700px] overflow-hidden transition-all duration-300 border border-neon hover:ring-2 hover:ring-neon hover:shadow-[0_0_20px_rgba(0,197,255,0.2)] [&:hover]:bg-black"
>
  <div className="absolute bottom-[-50px] right-[-50px] w-[110px] h-[110px] bg-neon opacity-50 rounded-full animate-blob z-0 transition-all duration-300 group-hover:opacity-100 group-hover:scale-110"></div>

  {packageInfo.category && (
    <div className="absolute top-2 left-2 px-1 text-neon border border-neon opacity-100 rounded-xl flex items-center justify-center text-center text-10px font-bold transition-colors duration-300 group-hover:text-black group-hover:bg-neon">
      {packageInfo.category}
    </div>
  )}

  {packageInfo.ribbonText && (
    <div className="absolute top-4 right-[-40px] w-[160px] rotate-45 bg-neon text-black text-center text-xs font-bold py-1 shadow-md z-10 transition-colors duration-300  group-hover:text-black">
      {packageInfo.ribbonText}
    </div>
  )}
  ...
  <div className="relative mb-2">
    <BodyText
      ...
      className="font-bold text-center h-[100px] xl:h-[140px] overflow-hidden transition-colors duration-300 group-hover:text-white text-black"
    />
    <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-[100px] h-[4px] bg-black rounded-full transition-colors duration-300 group-hover:bg-white"></div>
  </div>

  <BodyText
    text={packageInfo.description}
    centered={true}
    className="text-gray-600 text-sm mb-4 h-[50px] xl:h-[60px] leading-tight overflow-hidden transition-colors duration-300 group-hover:text-white"
  />
  ...
  <div className="flex flex-row justify-center items-end gap-x-2 mb-3 h-[70px]">
    <div className="flex flex-col justify-end min-w-[200px] items-center">
      <div className="flex items-end gap-x-1">
        <span className="text-sm font-medium transition-colors duration-300 group-hover:text-white">{currencyCode}</span>
        <span className="text-100px xl:text-60px leading-none font-bold transition-colors duration-300 group-hover:text-white">
          {formattedPrice}
        </span>
      </div>
    </div>
    ...
    {shouldShowToggle && (
      <div className="flex items-center justify-center w-[50px]">
        <button
          ...
          className="flex items-center justify-center px-2 py-1 text-10px font-medium rounded bg-gray-100 text-black hover:bg-neon hover:text-black transition-colors duration-200 group-hover:bg-white group-hover:text-black group-hover:hover:bg-neon group-hover:hover:text-black"
  ...
  <div className="flex-1 overflow-y-auto my-4 h-[200px] rounded-md package-scrollbar text-black transition-colors duration-300 group-hover:text-white">
    <div className="space-y-4 max-w-[250px] mx-auto">
      {Object.entries(packageInfo.features).map(([section, items], i) => (
        <div key={i}>
          <h4 className="text-md font-bold text-black mb-1 transition-colors duration-300 group-hover:text-white">{section}:</h4>
            ...
            {items.map((item, idx) => (
              <li key={idx} className="flex items-start space-x-2">
                ...
                <BodyText
                  text={item}
                  centered={false}
                  className="leading-snug text-sm pr-4 transition-colors duration-300 group-hover:text-white"
                />
              </li>
            ))}
          </ul>
  ...
  <div className="text-center mt-auto pt-10">
    <Button
      ...
      className="transition-colors duration-300 group-hover:text-black group-hover:bg-neon"
    />
    <div className="flex items-center justify-center gap-2 mt-2 text-sm text-black transition-colors duration-300 group-hover:text-white">
      {contactDetails
